...
const formatScheduledDate = (scheduledAt, scheduledDate, timezone) => {
  const source = scheduledAt || scheduledDate;
...
const formatScheduledTime = (scheduledAt, scheduledTime, timezone) => {
  if (scheduledTime) {
...
const ApplicationComplete = () => {
  ...
  useEffect(() => {
    if (!selectedAgent || (!scheduledDate && !scheduledAt)) {
      router.push("/");
    }
  }, [router, scheduledAt, scheduledDate, selectedAgent]);

  const handleDoneClick = () => {
    sessionStorage.removeItem("appointmentData");
    window.location.href = "https://www.adecco.com/";
  };

  const displayDate = useMemo(
    () => formatScheduledDate(scheduledAt, scheduledDate, timezone),
    [scheduledAt, scheduledDate, timezone]
  );

  const displayTime = useMemo(
    () => formatScheduledTime(scheduledAt, scheduledTime, timezone),
          ...
          <div className="bg-white max-w-5xl w-full rounded-lg sm:rounded-xl border overflow-hidden">
            ...
            <div className="p-4 sm:p-6 lg:p-10 text-center flex flex-col gap-6 items-center py-8 sm:py-12 lg:py-16">
              ...
              <p className="text-[10px] sm:text-xs text-gray-400 mt-4 px-4">
                {inviteeName
                  ? `A confirmation has been sent for ${inviteeName}. Please check your email for the meeting link and calendar invite.`
